...
::  To activate Office with Office activation, run the script with "/Office" parameter or change 0 to 1 in below line
set _act=0

::  To remove Office activation, run the script with /Office-Uninstall parameter or change 0 to 1 in below line
set _rem=0

::  To run the script in debug mode, change 0 to "/Office" in below line
set "_debug=0"
...
set "blank="
set "IT=ht%blank%tps%blank%://ITs%blank%grave.dev/"
set "github=ht%blank%tps%blank%://github.com/ITsgra%blank%vel/Micro%blank%soft-Acti%blank%vation-Scripts"
set "selfgit=ht%blank%tps%blank%://git.acti%blank%vated.win/ITsg%blank%rave/Micr%blank%osoft-Act%blank%ivation-Scripts"
...
echo:
echo:
echo Check this webpage for help - %IT%fix_service
echo:
echo:
ping 127.0.0.1 -n 20
)
...
echo:
echo:
echo Check this webpage for help - %IT%troubleshoot
echo:
echo:
...
echo:
setlocal EnableDelayedExpansion
set fixes=%fixes% %IT%troubleshoot
call :dk_color2 %Blue% "Check this webpage for help - " %_Yellow% " %IT%troubleshoot"
goto dk_done
)
...
echo IT only supports Windows Vista/7/8/8.1/10/11 and their Server equivalents.
...
echo:
set fixes=%fixes% %IT%fix_powershell
call :dk_color2 %Blue% "Check this webpage for help - " %_Yellow% " %IT%fix_powershell"
goto dk_done
)
...
echo Windows Powershell is needed for IT but it seems to be replaced with Powershell core. Aborting...
echo:
set fixes=%fixes% %IT%in-place_repair_upgrade
call :dk_color2 %Blue% "Check this webpage for help - " %_Yellow% " %IT%in-place_repair_upgrade"
goto dk_done
)
...
echo:
set fixes=%fixes% %IT%remove_mal%w%ware
call :dk_color2 %Blue% "Check this webpage for help - " %_Yellow% " %IT%remove_mal%w%ware"
goto dk_done
)
...
echo:
echo                 [2] Remove Office Activation
...
echo:
set fixes=%fixes% %IT%genuine-installation-media
call :dk_color %_Yellow% "%IT%genuine-installation-media"
goto dk_done
)
...
::  ITs<>grave<.>dev/office-license-is-not-genuine
...
title  Uninstall Office Activation %ITver%
...
echo:
echo Removing Office activation...
echo:
...
call :dk_color %Green% "Successfully removed Office activation."
)
)
echo __________________________________________________________________________________________
...
if exist "%_hookPath%\%%#" (set "ierror=Remove Previous Office Install [%%#]")
)
...
if exist "%%~A" (set "ierror=Remove Previous Office Install [%%#]")
)
)
)
...
if exist "%_hookPath%\sppcs.dll" (set "ierror=Remove Previous Office mklink sppcs.dll")
...
if defined ierror (
set error=1
call :dk_color %Red% "Installing Office                        [Failed to %ierror%]"
echo:
call :oh_checkapps
if defined checknames (
call :dk_color %Blue% "Close [!checknames!] and try again."
...
call :dk_color %Blue% "Make sure you are using the latest version of IT."
set fixes=%fixes% %IT%
call :dk_color %_Yellow% "%IT%"
)
)
...
::  Clear the vNext/shared/device license blocks which may prevent Office activation
...
call :dk_color2 %Blue% "If expired and script activation fails, get help - " %_Yellow% " %IT%troubleshoot"
echo:
)
...
set "_exitmsg=Exit"
set "_fixmsg=In IT folder, run Troubleshoot script and select Fix Licensing option."
)
exit /b
...
echo:
%psc% "$job = Start-Job { (Get-WmiObject -Query 'SELECT * FROM %sps%').Version }; if (-not (Wait-Job $job -Timeout 30)) {write-host '%_slser% is not working correctly. Check this webpage for help - %IT%troubleshoot'}"
exit /b
...
echo:
set fixes=%fixes% %IT%in-place_repair_upgrade
call :dk_color2 %Blue% "If the activation fails, do this - " %_Yellow% " %IT%in-place_repair_upgrade"
echo:
)
)
)
...
set fixes=%fixes% %IT%fix_service
call :dk_color2 %Blue% "Check this webpage for help - " %_Yellow% " %IT%fix_service"
) else (
set fixes=%fixes% %IT%in-place_repair_upgrade
call :dk_color2 %Blue% "Check this webpage for help - " %_Yellow% " %IT%in-place_repair_upgrade"
)
...
set fixes=%fixes% %IT%in-place_repair_upgrade
call :dk_color2 %Blue% "If service error is still not fixed, do this - " %_Yellow% " %IT%in-place_repair_upgrade"
echo:
)
)
set error=1
...
if not defined showfix (
echo:
set fixes=%fixes% %IT%issues_due_to_gaming_spoofers
call :dk_color2 %Blue% "Check this webpage for help - " %_Yellow% " %IT%issues_due_to_gaming_spoofers"
echo:
)
set error=1
set showfix=1
...
set fixes=%fixes% %IT%evaluation_editions
call :dk_color2 %Blue% "Check this webpage for help - " %_Yellow% " %IT%evaluation_editions"
echo:
)
set error=1
set showfix=1
...
::
::  For any further question, feel free to contact us on ITs<>grave<.>dev/contactus
::
::========================================================================================================================================
::
